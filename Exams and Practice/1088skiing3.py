...
for spot in sorting:
    if 0<spot[0]-1<r+1 and 0<spot[1]<c+1 and spot[2]<h[spot[0]-1][spot[1]]:
        res[spot[0]-1][spot[1]] = max(res[spot[0]][spot[1]] + 1, res[spot[0]-1][spot[1]])
    if 0<spot[0]+1<r+1 and 0<spot[1]<c+1 and spot[2]<h[spot[0]+1][spot[1]]:
        res[spot[0]+1][spot[1]] = max(res[spot[0]][spot[1]] + 1, res[spot[0]+1][spot[1]])
    if 0<spot[0]<r+1 and 0<spot[1]-1<c+1 and spot[2]<h[spot[0]][spot[1]-1]:
        res[spot[0]][spot[1]-1] = max(res[spot[0]][spot[1]] + 1, res[spot[0]][spot[1]-1])
    if 0<spot[0]<r+1 and 0<spot[1]+1<c+1 and spot[2]<h[spot[0]][spot[1]+1]:
        res[spot[0]][spot[1]+1] = max(res[spot[0]][spot[1]] + 1, res[spot[0]][spot[1]+1])
max_group = []
for i in res:
